...
public class SessionController {
    ...
    /**
     * Notify the user sessions.
     *
     * @param username the username
     * @throws ServiceException the service exception
     */
    public void notifySession(String username, String payload) throws ServiceException {
        Collection<ClientSession> clientSessions = SessionManager.getInstance().getSessions(username);

        for (ClientSession session : clientSessions)
        {
            routeMessage(session, payload);
        }
    }
     ...
     * @throws ServiceException the service exception
     */
    public void notifySessions(String payload) throws ServiceException {
        Collection<ClientSession> clientSessions = SessionManager.getInstance().getSessions();
    ...
    private void routeMessage(ClientSession session, String payload)
    {
        Message message = new Message();
        message.setFrom("admin@" + XMPPServer.getInstance().getServerInfo().getXMPPDomain());
        message.setTo(session.getAddress());
        message.setType(Message.Type.headline);

        Element properties = message.addChildElement("properties", "http://www.jivesoftware.com/xmlns/xmpp/properties");
        Element property = properties.addElement("property");
        property.addElement("name").setText("data");

        Element value = property.addElement("value");
        value.addAttribute("type", "string");
        value.setText(payload);
