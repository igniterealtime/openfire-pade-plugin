...
public class ConferenceMember implements TreatmentDoneListener,
	...
	try {
	    while (true) {
		    ...
		    //Logger.println("Call " + cp + " Opened datagram channel " + nextRtpPort);
		...
		try {
		    ...
		    Logger.println("Call " + cp + " RTCP Port " + (localPort + 1));
	    ...
	    byte receivePayload, byte telephoneEventPayload,    InetSocketAddress rtcpAddress)
	{
		...
		if (cp.getProtocol() != null && "WebRtc".equals(cp.getProtocol()) == false && "Rtmfp".equals(cp.getProtocol()) == false && "Speaker".equals(cp.getProtocol()) == false && "Multicast".equals(cp.getProtocol()) == false)
		{
			if (rtcpAddress != null) {
