...
public class MemberReceiver implements MixDataSource, TreatmentDoneListener {
	...
	if (cp.getProtocol() != null && ("WebRtc".equals(cp.getProtocol()) || "Rtmfp".equals(cp.getProtocol()) || "Speaker".equals(cp.getProtocol()) || "Multicast".equals(cp.getProtocol())))
	{
	...
	if (payload == RtpPacket.COMFORT_PAYLOAD || payload == 19) {
	    ...
	    try {
	        rtpTimestampAdjustment = receiveMedia(receivedData, length);
                ...
                Logger.println("decorder failed: " + e.getMessage());
                e.printStackTrace();
	    ...
	    if (recordingType.equalsIgnoreCase("Rtp")) {
            ...
            synchronized (recordingLock) {
                ...
                try {
					...
					try {
						m = SdpManager.findMediaInfo(RtpPacket.PCMU_ENCODING, conferenceManager.getMediaInfo().getSampleRate(), conferenceManager.getMediaInfo().getChannels());

					} catch (ParseException e1) {

						Logger.println("Can't record rtp to " + recordingFile + " " + e1.getMessage());
