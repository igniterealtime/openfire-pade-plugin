...
public class SipUtil {
	...
	supportedMedia.add(new MediaInfo(
        ...
        supportedMedia.add(new MediaInfo(
            (byte)100, RtpPacket.PCM_ENCODING, 8000, 1, false));
...
if (false) {
    ...
    public ClientTransaction sendInvite(CallParticipant cp, InetSocketAddress isa) throws InvalidArgumentException, SipException, ParseException
    {
    ...
    public ClientTransaction sendInvite(CallParticipant cp, String sdp) throws ParseException, InvalidArgumentException, SipException
    {
		...
		String fromNumber = cp.getFromPhoneNumber();
		String toNumber = cp.getPhoneNumber();
		String transport = cp.getTransport();

		if (fromNumber.indexOf("tel:") == 0) fromNumber = fromNumber.substring(4);

		Logger.println("XXX from = " + fromName + " " + fromNumber);
		...
		if (toNumber.indexOf("sip:") == 0) {
			...
			if (SipServer.clientRegistrations.containsKey(toNumber))
			{
				toNumber = SipServer.clientRegistrations.get(toNumber);
				voipGateway = JiveGlobals.getProperty("xmpp.domain", "localhost");
				toAddress = (SipURI)addressFactory.createAddress(toNumber).getURI();
				requestURI = toAddress;

				Logger.println("Call " + cp + " Using client registration " + toNumber);
		...
		{
			try {
				Logger.println("XXXX gatewayRequired 14 " + obProxy);
		...
		if (cp.isAutoAnswer())
		{
			...
			try {
				...
				invite.addHeader(headerFactory.createHeader("Call-Info", "Auto Answer"));
				invite.addHeader(headerFactory.createHeader("Call-Info", "answer-after=0"));
				invite.addHeader(headerFactory.createHeader("X-AUTOANSWER", "TRUE"));
		...
		if (cp.getConferenceId() != null && cp.getConferenceHeaderName() != null)
		{
			invite.addHeader(headerFactory.createHeader(cp.getConferenceHeaderName(), cp.getConferenceId()));
		}
