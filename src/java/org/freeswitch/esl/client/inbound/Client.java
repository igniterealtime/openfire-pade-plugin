...
public class Client
{
     ...
     * @return a {@link CommandResponse} with the server's response.
     */
    public EslMessage sendMessage( SendMsg sendMsg )
    {
        checkConnected();

        InboundClientHandler handler = (InboundClientHandler)channel.getPipeline().getLast();
        return handler.sendSyncMultiLineCommand( channel, sendMsg.getMsgLines() );
    }

    public EslMessage setBlf(String endpoint, String action)
    {
        checkConnected();
        ...
        msgLines.add("sendevent PRESENCE_IN");
        msgLines.add("proto: sip");
        msgLines.add("from: " + endpoint);
        msgLines.add("login: " + endpoint);
        msgLines.add("event_type: presence");
        msgLines.add("alt_event_type: dialog");
        msgLines.add("Presence-Call-Direction: outbound");
        msgLines.add("answer-state: " + ("flash".equals(action) ? "early" : ("on".equals(action) ? "confirmed" : "terminated")));

        return handler.sendSyncMultiLineCommand(channel, msgLines);
    }
